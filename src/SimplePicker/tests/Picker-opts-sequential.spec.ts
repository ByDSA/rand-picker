...
it("on trying to pick more sequential items than existing in the picker, picked elements  should be the same as data array", () => {
  const picker = pickerNumbers1();
  const times = 30;
  const picked = picker.pick(times, pickOptions);

  expect(picked).toEqual(picker.data);
} );
describe("put", ()=>{
...
it("on n=2, second picked element always should be next to first", () => {
  const picker = pickerNumbers1();
  const n = 2;
  const picked: number[][] = [];

  for (let i = 0; i < 300; i++) {
    const p = picker.pick(n, pickOptions);

    picked.push(p);
  }

  picked.forEach((p: number[]) => {
